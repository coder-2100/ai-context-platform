...
describe('端到端流程', () => {
  it('完整的 init → add → build → remove 流程', async () => {
    // 1. init
    await initCommand({ projectDir: TEST_DIR, projectName: 'e2e-project', assetsDir: ASSETS_DIR })
    ...
    const listAfterAdd = await listCommand({ projectDir: TEST_DIR, assetsDir: ASSETS_DIR })
    ...
    const listAfterRemove = await listCommand({ projectDir: TEST_DIR, assetsDir: ASSETS_DIR })
