...
describe('initCommand', () => {
  ...
  it('config.yaml 包含项目名', async () => {
    await initCommand({ projectDir: TEST_DIR, projectName: 'my-app', assetsDir: ASSETS_DIR })
    const config = readFileSync(join(TEST_DIR, '.ai', 'config.yaml'), 'utf-8')
    expect(config).toContain('my-app')
  })

  it('已初始化时抛出错误', async () => {
    await initCommand({ projectDir: TEST_DIR, projectName: 'first', assetsDir: ASSETS_DIR })
    await expect(initCommand({ projectDir: TEST_DIR, projectName: 'second', assetsDir: ASSETS_DIR })).rejects.toThrow()
  })
