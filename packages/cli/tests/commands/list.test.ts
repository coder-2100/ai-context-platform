...
describe('listCommand', () => {
  it('返回已安装包列表', async () => {
    ...
    const result = await listCommand({ projectDir: TEST_DIR, assetsDir: ASSETS_DIR })
  ...
  it('verbose 模式返回完整信息', async () => {
    ...
    const result = await listCommand({ projectDir: TEST_DIR, assetsDir: ASSETS_DIR, verbose: true })
